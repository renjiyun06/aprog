...
export function makeProgram(procTitle: string, seed: ProcState[], pidStart = 5000): Component<ProgramProps> {
  return (props) => {
    ...
    const shownProcs = createMemo<ProcInfo[]>(() =>
      procs().map((p) => ({ pid: p.pid, name: p.name, version: p.version, dot: p.dot, active: p.pid === activeId() })),
    );
    ...
    /* lifecycle: hibernate = drop the sandbox (no more live conversation);
       wake = re-associate a sandbox so the process can run again. */
    const setDot = (pid: number, dot: ProcState['dot']) =>
      setProcs((list) => list.map((p) => (p.pid === pid ? { ...p, dot } : p)));
    const hibernate = (pid: number) => setDot(pid, 'hibernating');
    const wake = (pid: number) => setDot(pid, 'running');
    ...
    const spawn = (name: string) => {
      ...
      const fresh: ProcState = {
        pid, name, version: '0.4.0', dot: 'running', dir: `~/.aprog/${pid}`,
    ...
    return (
      <ProgramShell
        ...
        tree={active()?.tree ?? []}
        onOpenFile={openFile}
        onSyncTree={() => {/* mock: real impl re-reads the dir tree from the sandbox via provider fs */}}
        onSyncFile={(_path) => {/* mock: real impl re-reads this file's content from the sandbox */}}
