...
import { windowOpacity, taskbarOpacity, setWindowOpacity, setTaskbarOpacity } from '../../stores/appearance';
          ...
          <Show when={section() === 'personalization'}>
            ...
            <section class="set-card">
              <div class="set-slider-row">
                <div class="set-slider-label">
                  <span class="k">窗口透明度</span>
                  <span class="v mono">{Math.round(windowOpacity() * 100)}%</span>
                </div>
                <input type="range" class="set-slider" min="0.4" max="1" step="0.01"
                  value={windowOpacity()}
                  onInput={(e) => setWindowOpacity(Number(e.currentTarget.value))} />
              </div>
              <div class="set-slider-row">
                <div class="set-slider-label">
                  <span class="k">状态栏透明度</span>
                  <span class="v mono">{Math.round(taskbarOpacity() * 100)}%</span>
                </div>
                <input type="range" class="set-slider" min="0.3" max="1" step="0.01"
                  value={taskbarOpacity()}
                  onInput={(e) => setTaskbarOpacity(Number(e.currentTarget.value))} />
              </div>
              <div class="set-hint">数值越低越透明，透出桌面背景。</div>
            </section>
          </Show>
